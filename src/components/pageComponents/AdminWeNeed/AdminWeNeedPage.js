...
export default function AdminWeNeedPage() {
  ...
  async function updateStatus(id, newStatus) {
    ...
    const prev = items;
    setItems((cur) =>
      cur.map((it) => (it.id === id ? { ...it, status: newStatus } : it))
    );
    try {
      // If your backend exposes /approve/ or /reject/ endpoints, swap this PATCH for those.
              ...
              <div className="awn-tbody" role="rowgroup">
                {filtered.map((it) => (
                  <div className="awn-tr" role="row" key={it.id}>
                    <div className="awn-td">{it.created_at ? new Date(it.created_at).toLocaleString() : ""}</div>
                    <div className="awn-td">{it.student?.name || "Anonymous"}</div>
                    <div className="awn-td">{it.student?.number || ""}</div>
                    <div className="awn-td"><span className="chip">{it.category}</span></div>
